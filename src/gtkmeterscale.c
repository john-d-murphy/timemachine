...
GtkType gtk_meterscale_get_type() {
  ...
  if (!meterscale_type) {
    ...
    meterscale_type = gtk_type_unique(gtk_widget_get_type(), &meterscale_info);
  }
...
static void meterscale_draw_notch_label(GtkMeterScale* meterscale, float db,
  ...
  if (last_label_rect) {
    ...
    if (vertical && last_label_rect->y < y + rect.height + 2 &&
        last_label_rect->y + last_label_rect->height + 2 > y) {
      return;
    }
    if (!vertical && last_label_rect->x < x + rect.width + 2 &&
        last_label_rect->x + last_label_rect->width + 2 > y) {
      return;
    }
...
static void meterscale_draw_notch(GtkMeterScale* meterscale, float db,
  ...
  if (meterscale->direction & GTK_METERSCALE_LEFT) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, 0, pos,
                       mark, 1);
  }
  ...
  if (meterscale->direction & GTK_METERSCALE_TOP) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, pos + 1,
                       1, 1, mark);
  }
